				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						$sql_ary = array(
							...
							'user_avatar'		=> ($auth->acl_get('u_chgavatar') && $user->data['user_avatar_type'] == 9) ? $config['server_protocol'] . 'www.gravatar.com/avatar/' . md5(strtolower(trim((($auth->acl_get('u_chgemail')) ? $data['email'] : $user->data['user_email'])))) . '?d=identicon&amp;s=' . min($config['avatar_max_width'], $config['avatar_max_height']) : $user->data['user_avatar'],
						);
			...
			case 'avatar':
				...
				if ($submit)
				{
					if (check_form_key('ucp_avatar'))
					{
						...
						$gravatar = (!empty($_POST['gravatar'])) ? true : false;
				...
				$template->assign_vars(array(
					...
					'S_GRAVATAR'	=> ($user->data['user_avatar_type'] == 9) ? 'yes' : '',
...
'S_ALLOW_GRAVATAR'      => (!empty($config['allow_gravatar'])) ? (int) $config['allow_gravatar'] : 0,
